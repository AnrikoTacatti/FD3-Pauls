        ...
        for (let i = 1; i <= length; i++) {
            ...
            tasklistitem.push(
                <TaskItem data={listtask[listtask.length - i]} keychapter={keychapter} keyitem={keyitem} key={keyitem} index={i}
                    attrdata={this.state.locationPathname !== undefined ? this.state.locationPathname : "all"}
                />
            )
        }
            ...
            <React.Fragment>
                <div className="task-chapter task-chapter--main" >
                    ...
                    <div className="book" style={{ "animationDelay": `1s` }}>
                        ...
                        <textarea className="book__text" ref={c => this.bookText = c} defaultValue={this.state.Book !== undefined && this.state.Book.text} placeholder="Запишите что-нибудь"></textarea>
                    </div>
